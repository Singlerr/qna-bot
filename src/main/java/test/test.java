...
public class test {
    ...
    public void doJob() throws Exception {
        ...
        int numEpochs = 100;							// 총 훈련 에포크 수
        int generateSamplesEveryNMinibatches = 4;  // 신경망에서 샘플을 생성하는 빈도, 1000 문자 / 50 tbptt 길이 : minibatch 당 20 매개 변수 업데이트
        int nSamplesToGenerate = 20;					// 각 학습 기간 이후에 생성 할 샘플 수
        int nCharactersToSample = 500;				// 생성할 샘플의 길이
        ...
        CharacterIterator iter = getQuestionIterator(miniBatchSize,exampleLength);
        ...
        MultiLayerConfiguration conf = new NeuralNetConfiguration.Builder()
                ...
                .layer(2, new RnnOutputLayer.Builder(LossFunction.MCXENT).activation(Activation.SOFTMAX)        //MCXENT + softmax for classification
                        .nIn(lstmLayerSize).nOut(iter.nOutPut()).build())
        ...
        for( int i=0; i<layers.length; i++ ){
            int nParams = Integer.valueOf(layers[i].numParams());
        ...
        BufferedWriter writer = Files.newWriter(new File("samples.txt"),Charset.forName("euc-kr"));
        for( int i=0;i<10; i++ ){
            while(iter.hasNext()){
     ...
     */
    public static CharacterIterator getQuestionIterator(int miniBatchSize, int sequenceLength) throws Exception{
...
**/
        ...
        System.out.println("Loading Korean Characters from URL");
        URL korean = new URL("https://raw.githubusercontent.com/Singlerr/DeepLearning/main/koreanChars.txt");
        BufferedReader reader = new BufferedReader(new InputStreamReader(korean.openConnection().getInputStream(),"UTF8"));
        StringBuilder builder = new StringBuilder();
        String line;
        while ((line=reader.readLine()) != null){
            builder.append(line);
        }
        List<Character> characters = new ArrayList<>(Chars.asList(validCharacters));
        for(char c : builder.toString().toCharArray())
            characters.add(c);
        validCharacters =  Chars.toArray(characters);
        return new CharacterIterator("data.csv", Charset.forName("UTF-8"),
