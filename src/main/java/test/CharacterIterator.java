...
public class CharacterIterator implements DataSetIterator {
	...
	public CharacterIterator(String textFilePath, Charset textFileEncoding, int miniBatchSize, int exampleLength,
                             char[] validCharacters, Random rng) throws IOException, InterruptedException {
		...
		CSVSentenceIterator iterator = new CSVSentenceIterator(new File(textFilePath),0);
		while (iterator.hasNext()) {
			lines.add(iterator.nextSentence());
			outcomes++;
		}
	...
	public DataSet next(int num) {
		...
		INDArray labels = Nd4j.create(new int[]{currMinibatchSize,outcomes,exampleLength}, 'f');
